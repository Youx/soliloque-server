...
static void s_resp_chan_name_changed(struct player *pl, struct channel *ch, char *name)
{
	...
	if (data == NULL) {
		ERROR("s_resp_chan_name_changed, packet allocation failed : %s.", strerror(errno));
		return;
	}
	ptr = data;
...
static void s_resp_chan_topic_changed(struct player *pl, struct channel *ch, char *topic)
{
	...
	data = (char *)calloc(data_size, sizeof(char));
	if (data == NULL) {
		ERROR("s_resp_chan_topic_changed, packet allocation failed : %s.", strerror(errno));
		return;
	}
	ptr = data;
...
static void s_resp_chan_desc_changed(struct player *pl, struct channel *ch, char *desc)
{
	...
	data = (char *)calloc(data_size, sizeof(char));
	if (data == NULL) {
		ERROR("s_resp_chan_desc_changed, packet allocation failed : %s.", strerror(errno));
		return;
	}
	ptr = data;
...
static void s_notify_channel_flags_codec_changed(struct player *pl, struct channel *ch)
{
	...
	data = (char *)calloc(data_size, sizeof(char));
	if (data == NULL) {
		ERROR("s_notify_channel_flags_codec_changed, packet allocation failed : %s.", strerror(errno));
		return;
	}
	ptr = data;
...
void *c_req_change_chan_pass(char *data, unsigned int len, struct player *pl)
{
	...
	if (ch != NULL && player_has_privilege(pl, SP_CHA_CHANGE_PASS, ch) && ch->parent == NULL) {
		INFO("Change channel password : %s->%s", ch->name, password);
...
static void s_notify_channel_order_changed(struct player *pl, struct channel *ch)
{
	...
	data = (char *)calloc(data_size, sizeof(char));
	if (data == NULL) {
		ERROR("s_notify_channel_order_changed, packet allocation failed : %s.", strerror(errno));
		return;
	}
	ptr = data;
...
static void s_notify_channel_max_users_changed(struct player *pl, struct channel *ch)
{
	...
	data = (char *)calloc(data_size, sizeof(char));
	if (data == NULL) {
		ERROR("s_notify_channel_max_users_changed, packet allocation failed : %s.", strerror(errno));
		return;
	}
	ptr = data;
