...
struct server *new_server()
{
	...
	if (serv == NULL) {
		ERROR("new_server, calloc failed : %s.", strerror(errno));
		return NULL;
	}
...
int add_channel(struct server *serv, struct channel *chan)
{
	...
	if (used_ids == NULL) {
		ERROR("add_channel, used_ids allocation failed : %s.", strerror(errno));
		return 0;
	}
...
int add_player(struct server *serv, struct player *pl)
{
	...
	if (used_ids == NULL) {
		ERROR("add_player, used_ids allocation failed : %s.", strerror(errno));
		return 0;
	}
...
static void *server_run(void *args)
{
	...
	while (1) {
		...
		case 0:
			...
			break;
		case -1:
			ERROR("Error occured while polling : %s", strerror(errno));
			break;
		default:
